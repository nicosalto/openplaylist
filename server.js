...
io.on('connection', function (socket) {
    ...
    socket.on('skip', function (data) {
        ...
        if (links.length > 0){
            ...
            if(playing.youtubeid == data.youtubeid || playing == 'none' || data.from == 'user' ){
                if (data.from == 'user'){
                    broadcast('history', {'message' : ' skipped :'+playing['title'] ,'time': getthetime(),'user':data.user } ); //add link to history
                }
                playbutton = 'pause';
                ...
                broadcast('history', {'message' : ' playing :'+playing['title'] ,'time': getthetime(),'user':data.user } ); //add link to history
            }
        }
    });
